...
'''
Base python library for GRV tooling.
...
Testing:
    ...
    'grv test-aws -L', see 'grv test-aws -h' for more info.
...
def fetch_grv_meta(grv_id, region=None):
    ...
    Returns: Flat dict of values derived from AWS tags for GRV VPC.
    '''
    returnmeta = {}
    _found = False
    _next_token = ''
...
def validate_grv_id(grv_id):
    ...
    Args: string, grv name or vpc id.
...
def list_logical_subnets(network):
    ...
    Args: network - string, a logical subnet name, (e.g. wan.grv_id.tld)
    ...
    try:
        ...
        try:
            physical_map = r53_lookup(name='physical_map.{}'.format(
                grvname_or_logicalname), dns_type='TXT')
        # Need to walk back the domain in list_logical_networks
        except IndexError or ValueError as err1:
            for nametry in domain_walkback(grvname_or_logicalname):
        ...
        for subname in list(map_dict.keys()):
            if subname.endswith(grvname_or_logicalname):
...
def list_vpc_subnets(grv_id):
    ...
    Args: grv_id (str): single VPC ID or Name, fetches only subnets it contains.
...
def list_grv_security_groups(grv_or_net_name=''):
    ...
    Args: grv_or_net_name - string, named grv, or logical network,
...
def list_grv_instances(grv_name='', phys_subnet='', logical_net='',):
    ...
    Returns: dict keyed by instance id, containing grv-enhanced AWS metadata.
    ...
    try:
        ...
        grv_class = r53_lookup(
            name="class.{}".format(grv_name), dns_type='TXT')[0]
...
def vpc_domains_enabled(grv_id):
    ...
    VPC attributes are set:
         EnableDnsHostnames
         EnableDnsSupport
    Args: grv_id string, vpc name or id
...
def fetch_account_id():
    ...
    https://addeparsre.signin.aws.amazon.com/console
...
def list_grvs_s3_buckets(grv_id=None, bucket_id=None):
    ...
    Returns:
        ...
        When no args, returns all grv-related buckets for the account.
        This function always returns only the buckets tagged with
        'grv_create_session_id', indicating they belong to a GRV.
        With grv_id specified, returns all buckets attached to or belonging to
        a given VPC.
    ...
    Bugs:
        ...
        and strips the result when grv_id or bucket_id are provided.
    '''
    grvbuckets = {}
    try:
        ...
        for bucketmeta in s3response['Buckets']:
            ...
            target_bucket[bucketmeta['Name']]['Tags'] = tag_response['TagSet']
            target_bucket[bucketmeta['Name']]['TagSane'] = aws_tags_dict(
                tag_response['TagSet'])

            # TODO: workspot: add VPC endpoint metadata here

            if target_bucket.get(bucketmeta['Name']):
                if 'grv_id' in list(target_bucket[bucketmeta['Name']]['TagSane'].keys()):
                    target_bucket[bucketmeta['Name']]['grv_id'] = \
                        target_bucket[bucketmeta['Name']]['TagSane']['grv_id']
                else:
                    target_bucket[bucketmeta['Name']]['grv_id'] = ''

            if 'grv_create_session_id' in list(target_bucket[bucketmeta['Name']]['TagSane'].keys()):
                target_bucket[bucketmeta['Name']]['grv_create_session_id'] = \
                    target_bucket[bucketmeta['Name']
                                  ]['TagSane']['grv_create_session_id']
            else:
                target_bucket[bucketmeta['Name']]['grv_create_session_id'] = ''

            if len(target_bucket[bucketmeta['Name']]['TagSane']) == 0:
                pass
            elif grv_id is not None:
                if grv_id == target_bucket[bucketmeta['Name']]['grv_id']:
                    grvbuckets.update(target_bucket)
            elif bucket_id is not None:
                if bucket_id in list(target_bucket.keys()):
        ...
        for logiconf in list(logical.keys()):
            ...
            if logiconf.startswith('vpc_supernet'):
                ...
                logical_name = "{0}.{1}.{2}".format(
                    shortname, grv_name, logical['grv_tld']).lower()
...
#
# ----------------------------------------------------------
def check_if_policy(arcade_name: str,
                    component_name: str):
    """
